...
const searchCustomers = () => {
    Inertia.get(route('manager.companylist.index'), {
        search: search.value,
                    ...
                    <div v-if="inertiaJobs.data.length" class="-m-2">
                        <template v-for="job in inertiaJobs.data" :key="job.id">
                            <div class="rounded-lg bg-gray-100 my-6 py-6 ">
                                ...
                                <div class="p-2 w-full">
                                    <Link as="button" :href="route('manager.company.show', { id: job.id })" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">詳細へ</Link>
                                </div>
                            </div>
                        </template>
                    </div>
